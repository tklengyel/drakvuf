...
void userhook::destroy_running_trap(drakvuf_t drakvuf, drakvuf_trap_t *trap)
{
    ...
    drakvuf_trap_info_t* info)
{
    ...
    if (rh_data->state == HOOK_FIRST_TRY)
    {
        ...
        {
            userhook_plugin->destroy_running_trap(drakvuf, info->trap);
            return VMI_EVENT_RESPONSE_NONE;
        }
    ...
    void* extra)
{
    ...
    if (!drakvuf_add_trap(drakvuf, trap))
        rh_data_t::free_trap(trap);
    this->running_traps.push_back(trap);
}
