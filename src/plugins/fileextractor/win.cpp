...
//
event_response_t win_fileextractor::openfile_cb(drakvuf_t drakvuf,
...
event_response_t win_fileextractor::createfile_cb(drakvuf_t drakvuf,
    drakvuf_trap_info_t* info)
{
    if (this->is_stopping())
        return VMI_EVENT_RESPONSE_NONE;
...
void win_fileextractor::createfile_cb_impl(drakvuf_t,
    drakvuf_trap_info_t* info,
...
event_response_t win_fileextractor::createfile_ret_cb(drakvuf_t,
    drakvuf_trap_info_t* info)
{
...
event_response_t win_fileextractor::writefile_cb(drakvuf_t,
    drakvuf_trap_info_t* info)
{
    if (drakvuf_lookup_injection(drakvuf, info))
        drakvuf_remove_injection(drakvuf, info);
...
win_fileextractor::error win_fileextractor::dispatch_task(
    vmi_instance_t vmi,
    drakvuf_trap_info_t* info,
    task_t& task)
{
...
win_fileextractor::error win_fileextractor::dispatch_pending(
    vmi_instance_t vmi,
    drakvuf_trap_info_t* info,
    task_t& task)
{
...
win_fileextractor::error win_fileextractor::dispatch_queryvolumeinfo(
    vmi_instance_t vmi,
    drakvuf_trap_info_t* info,
    task_t& task)
{
...
win_fileextractor::error win_fileextractor::dispatch_queryinfo(
    vmi_instance_t vmi,
    drakvuf_trap_info_t* info,
    task_t& task)
{
...
win_fileextractor::error win_fileextractor::dispatch_createsection(
    vmi_instance_t vmi,
    drakvuf_trap_info_t* info,
    task_t& task)
{
...
win_fileextractor::error win_fileextractor::dispatch_mapview(
    vmi_instance_t vmi,
    drakvuf_trap_info_t* info,
    task_t& task)
{
...
win_fileextractor::error win_fileextractor::dispatch_allocate_pool(
    vmi_instance_t vmi,
    drakvuf_trap_info_t* info,
    task_t& task)
{
...
win_fileextractor::error win_fileextractor::dispatch_memcpy(
    vmi_instance_t vmi,
    drakvuf_trap_info_t* info,
    task_t& task)
{
...
win_fileextractor::error win_fileextractor::dispatch_unmapview(
    vmi_instance_t vmi,
    drakvuf_trap_info_t* info,
    task_t& task)
{
...
win_fileextractor::error win_fileextractor::dispatch_close_handle(
    vmi_instance_t vmi,
    drakvuf_trap_info_t* info,
    task_t& task)
{
...
bool win_fileextractor::inject_queryvolumeinfo(drakvuf_trap_info_t* info,
    vmi_instance_t vmi,
    task_t& task)
{
...
bool win_fileextractor::inject_queryinfo(drakvuf_trap_info_t* info,
    vmi_instance_t vmi,
    task_t& task)
{
    // Remove stack arguments and home space from previous injection
...
bool win_fileextractor::inject_createsection(drakvuf_trap_info_t* info,
    vmi_instance_t vmi,
    task_t& task)
{
    // Remove stack arguments and home space from previous injection
...
bool win_fileextractor::inject_mapview(drakvuf_trap_info_t* info,
    vmi_instance_t vmi,
    task_t& task)
{
    // Remove stack arguments and home space from previous injection
...
bool win_fileextractor::inject_allocate_pool(drakvuf_trap_info_t* info,
    vmi_instance_t vmi,
    task_t& task)
{
    // Remove stack arguments and home space from previous injection
...
bool win_fileextractor::inject_memcpy(drakvuf_trap_info_t* info,
    vmi_instance_t vmi,
    task_t& task)
{
    // Remove stack arguments and home space from previous injection
...
bool win_fileextractor::inject_unmapview(drakvuf_trap_info_t* info,
    vmi_instance_t vmi,
    task_t& task)
{
    // Remove stack arguments and home space from previous injection
...
bool win_fileextractor::inject_close_handle(drakvuf_trap_info_t* info,
    vmi_instance_t vmi,
    task_t& task)
{
    // Remove stack arguments and home space from previous injection
...
void win_fileextractor::check_stack_marker(
    drakvuf_trap_info_t* info,
    vmi_lock_guard& vmi,
    task_t* task)
{
...
bool win_fileextractor::get_file_object_handle_count(drakvuf_trap_info_t* info,
    handle_t handle,
...
bool win_fileextractor::get_file_object_flags(drakvuf_trap_info_t* info,
    vmi_instance_t vmi,
    handle_t handle,
    uint64_t* flags)
{
...
bool win_fileextractor::get_write_offset(vmi_instance_t vmi, drakvuf_trap_info_t* info, addr_t offset_addr, uint64_t* write_offset)
{
    ACCESS_CONTEXT(ctx,
        .translate_mechanism = VMI_TM_PROCESS_DTB,
        .dtb = info->regs->cr3,
...
void win_fileextractor::calc_checksum(task_t& task)
{
    auto file = get_data_filename(this->dump_folder, task.idx);
    if (file.empty())
        return;
...
void win_fileextractor::save_file_metadata(drakvuf_trap_info_t* info,
    addr_t control_area,
    task_t& task)
{
...
bool win_fileextractor::save_file_chunk_rb(int file_sequence_number, uint64_t currentoffset,
    void* buffer,
    size_t size)
{
    auto file = get_data_filename(this->dump_folder, file_sequence_number);
...
void win_fileextractor::dump_mem_to_file(uint64_t cr3, addr_t str, int idx, uint64_t offset, size_t size)
{
    vmi_lock_guard vmi(drakvuf);
    ACCESS_CONTEXT(ctx,
        .translate_mechanism = VMI_TM_PROCESS_DTB,
...
uint64_t win_fileextractor::make_task_id(task_t& task)
{
    return make_task_id(task.target.ret_pid, task.handle);
}

void win_fileextractor::free_pool(addr_t va)
{
    for (auto pool: pools)
        if (va == pool.first)
        {
...
addr_t win_fileextractor::find_pool()
{
    for (auto pool: pools)
        if (pool.second)
        {
...
void win_fileextractor::free_resources(drakvuf_trap_info_t* info, task_t& task)
{
    PRINT_DEBUG(
...
void win_fileextractor::remove_task(drakvuf_trap_info_t* info, task_t& task,
...
void win_fileextractor::read_vm(vmi_instance_t vmi,
    drakvuf_trap_info_t* info,
    task_t& task)
{
...
bool win_fileextractor::is_handle_valid(handle_t handle)
{
    return handle && !VMI_GET_BIT(handle, 31);
}

task_t* win_fileextractor::close_cb_get_task(drakvuf_trap_info_t* info)
{
    task_t* task = nullptr;
    for (auto& i: tasks)
...
void win_fileextractor::close_cb_handle_unextracted(drakvuf_trap_info_t* info,
    task_t* task)
{
    vmi_lock_guard vmi(drakvuf);
...
void win_fileextractor::close_cb_handle_extracted(drakvuf_trap_info_t* info,
    task_t* task)
{
    // close task
    auto handle = drakvuf_get_function_argument(drakvuf, info, 1);
...
task_t* win_fileextractor::writefile_cb_get_task(drakvuf_trap_info_t* info)
{
    vmi_lock_guard vmi(drakvuf);
    ...
    , sequence_number()
{
    ...
    this->setinformation_hook = createSyscallHook("NtSetInformationFile", &win_fileextractor::setinformation_cb);
    this->writefile_hook = createSyscallHook("NtWriteFile", &win_fileextractor::writefile_cb);
    this->close_hook = createSyscallHook("NtClose", &win_fileextractor::close_cb);
    this->createsection_hook = createSyscallHook("ZwCreateSection", &win_fileextractor::createsection_cb);
    this->createfile_hook = createSyscallHook("NtCreateFile", &win_fileextractor::createfile_cb);
    this->openfile_hook = createSyscallHook("NtOpenFile", &win_fileextractor::openfile_cb);
}
...
bool win_fileextractor::stop_impl()
{
    if (!begin_stop_at)
