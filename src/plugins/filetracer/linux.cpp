...
std::string get_filepath(drakvuf_t drakvuf, drakvuf_trap_info_t *info, vmi_instance_t vmi, addr_t dentry_addr)
{
    ...
    std::string path_name = vmi_read_str(vmi, &ctx);
...
int get_file_info(drakvuf_t drakvuf, drakvuf_trap_info_t *info, linux_wrapper *lw, addr_t struct_addr = 0, std::string struct_name = "")
{
    ...
    lw->filename = get_filepath(drakvuf, info, vmi_lg.vmi, dentry_addr);
...
std::string read_filename(drakvuf_t drakvuf, drakvuf_trap_info_t *info, addr_t fileaddr)
{
    vmi_lock_guard vmi_lg(drakvuf);
    ...
    std::string filename = "";
    filename = vmi_read_str(vmi_lg.vmi, &ctx);
    return filename;
}
...
static event_response_t memfd_create_file_cb(drakvuf_t drakvuf, drakvuf_trap_info_t *info)
{
    ...
    */
    ...
    lw->filename = read_filename(drakvuf, info, file_name_addr);
    if (lw->filename.c_str())
        print_info(drakvuf, info, lw);
    delete lw;
    return VMI_EVENT_RESPONSE_NONE;
...
static event_response_t rename_file_cb(drakvuf_t drakvuf, drakvuf_trap_info_t *info)
{
    ...
    */
    ...
    addr_t dentry_addr = drakvuf_get_function_argument(drakvuf, info, 2);
    vmi_instance_t vmi = drakvuf_lock_and_get_vmi(drakvuf);
    std::string file_name = get_filepath(drakvuf, info, vmi, dentry_addr);
    drakvuf_release_vmi(drakvuf);
    if (!file_name.c_str())
        return VMI_EVENT_RESPONSE_NONE;
...
static event_response_t truncate_file_cb(drakvuf_t drakvuf, drakvuf_trap_info_t *info)
{
    ...
    */
    ...
    int64_t length = drakvuf_get_function_argument(drakvuf, info, 2);
    lw->args["length"] = std::to_string(length);
...
static event_response_t allocate_file_cb(drakvuf_t drakvuf, drakvuf_trap_info_t *info)
{
    ...
    */
    ...
    lw->args["offset"] = std::to_string(offset);
...
std::string convet_to_octal_string(int val)
{
    std::string oct = "";
    while (val)
        oct.push_back('0' + val % 8), val /= 8;
...
static event_response_t chmod_file_cb(drakvuf_t drakvuf, drakvuf_trap_info_t *info)
{
    ...
    */
    ...
    lw->args["new_permissions"] = convet_to_octal_string(new_mode & 0xfff);
    if (get_file_info(drakvuf, info, lw, path_struct, "path"))
        print_info(drakvuf, info, lw);
    delete lw;
    return VMI_EVENT_RESPONSE_NONE;
...
static event_response_t chown_file_cb(drakvuf_t drakvuf, drakvuf_trap_info_t *info)
{
    ...
    */
    ...
    lw->args["new_uid"] = std::to_string(new_uid);
    uint64_t new_gid = drakvuf_get_function_argument(drakvuf, info, 3);
    lw->args["new_gid"] = std::to_string(new_gid);
    if (get_file_info(drakvuf, info, lw, path_struct, "path"))
        print_info(drakvuf, info, lw);
    delete lw;
    return VMI_EVENT_RESPONSE_NONE;
...
static event_response_t utimes_file_cb(drakvuf_t drakvuf, drakvuf_trap_info_t *info)
{
    ...
    */
    ...
    drakvuf_release_vmi(drakvuf);
    if (time_sec)
        lw->args["time_sec"] = std::to_string(time_sec);
...
static event_response_t access_file_cb(drakvuf_t drakvuf, drakvuf_trap_info_t *info)
{
    ...
    */
    ...
    if (file_name_addr) // check for valid addr
    {
        lw->filename = read_filename(drakvuf, info, file_name_addr);
        if (lw->filename.c_str())
            print_info(drakvuf, info, lw);
...
static event_response_t symbolic_link_file_cb(drakvuf_t drakvuf, drakvuf_trap_info_t *info)
{
    ...
    */
    ...
    if (oldname_addr)
        lw->args["oldname"] = read_filename(drakvuf, info, oldname_addr);
    if (get_file_info(drakvuf, info, lw, dentry_addr, "dentry"))
        print_info(drakvuf, info, lw);
    delete lw;
    return VMI_EVENT_RESPONSE_NONE;
...
static event_response_t read_link_cb(drakvuf_t drakvuf, drakvuf_trap_info_t *info)
{
    ...
    */
    ...
    lw->args["buflen"] = std::to_string(buflen);
    if (get_file_info(drakvuf, info, lw, dentry_addr, "dentry"))
        print_info(drakvuf, info, lw);
    delete lw;
    return VMI_EVENT_RESPONSE_NONE;
                          ...
                          event_response_t (*hook_cb)(drakvuf_t drakvuf, drakvuf_trap_info_t *info))
{
    ...
    trap->breakpoint.pid = 0;
    trap->breakpoint.addr_type = ADDR_VA;
    trap->breakpoint.addr = syscall_addr + f->kaslr;
    trap->breakpoint.module = "linux";
    trap->type = BREAKPOINT;
    trap->data = f;
    ...
    if (drakvuf_add_trap(drakvuf, trap))
        f->traps = g_slist_prepend(f->traps, trap);
    else
    {
        printf("Failed to register trap!! %s\n", trap->name);
        free_trap(trap);
        throw - 1;
    }
}
...
{
    if (!drakvuf_get_kernel_struct_members_array_rva(drakvuf, linux_offset_names,__LINUX_OFFSET_MAX, f->offsets))
        throw - 1;

    drakvuf_trap_t *trap[22];
    for (int i = 0; i < 22; i++)
        trap[i] = g_slice_new0(drakvuf_trap_t);

    assert(sizeof(trap) / sizeof(trap[0]) > 21);

    // File operations
    register_trap(drakvuf, "do_filp_open", trap[0], f, open_file_cb);
    register_trap(drakvuf, "vfs_read", trap[1], f, read_file_cb);
    register_trap(drakvuf, "vfs_write", trap[2], f, write_file_cb);
    register_trap(drakvuf, "filp_close", trap[3], f, close_file_cb);
    register_trap(drakvuf, "vfs_llseek", trap[4], f, llseek_file_cb);
    register_trap(drakvuf, "__x64_sys_memfd_create", trap[5], f, memfd_create_file_cb);
    register_trap(drakvuf, "vfs_mknod", trap[6], f, mknod_file_cb);
    register_trap(drakvuf, "vfs_rename", trap[7], f, rename_file_cb);
    register_trap(drakvuf, "do_truncate", trap[8], f, truncate_file_cb);
    register_trap(drakvuf, "vfs_fallocate", trap[9], f, allocate_file_cb);

    // File Attributes
    register_trap(drakvuf, "chmod_common", trap[10], f, chmod_file_cb);
    register_trap(drakvuf, "chown_common", trap[11], f, chown_file_cb);
    register_trap(drakvuf, "utimes_common.isra.0", trap[12], f, utimes_file_cb);
    register_trap(drakvuf, "do_faccessat", trap[13], f, access_file_cb);

    // Directory Operations
    register_trap(drakvuf, "vfs_mkdir", trap[14], f, mkdir_cb);
    register_trap(drakvuf, "vfs_rmdir", trap[15], f, rmdir_cb);
    register_trap(drakvuf, "set_fs_pwd", trap[16], f, chdir_cb);
    register_trap(drakvuf, "set_fs_root", trap[17], f, chroot_cb);

    // Link Operations
    register_trap(drakvuf, "vfs_link", trap[18], f, link_file_cb);
    register_trap(drakvuf, "vfs_unlink", trap[19], f, unlink_file_cb);
    register_trap(drakvuf, "vfs_symlink", trap[20], f, symbolic_link_file_cb);
    register_trap(drakvuf, "vfs_readlink", trap[21], f, read_link_cb);
}
