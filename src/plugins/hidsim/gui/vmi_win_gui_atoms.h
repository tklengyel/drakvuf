...
struct atom_entry* parse_atom_entry(vmi_instance_t vmi, addr_t atom_addr)
{
    ...
    if (VMI_FAILURE == vmi_read_addr_va(vmi, atom_addr + symbol_offsets.atom_entry_hashlink_offset, 0, &entry->hashlink))
    {
        printf("Error reading HashLink at %" PRIx64 "\n", atom_addr + symbol_offsets.atom_entry_hashlink_offset);
        return NULL;
    }

    if (VMI_FAILURE == vmi_read_16_va(vmi, atom_addr + symbol_offsets.atom_entry_atom_offset, 0, (uint16_t*)&entry->atom))
    {
        printf("Error reading Atom at %" PRIx64 "\n", atom_addr + symbol_offsets.atom_entry_atom_offset);
        return NULL;
    }

    if (VMI_FAILURE == vmi_read_16_va(vmi, atom_addr + symbol_offsets.atom_entry_ref_count_offset, 0, (uint16_t*)&entry->ref_count))
    {
        printf("Error reading ReferenceCount at %" PRIx64 "\n", atom_addr + symbol_offsets.atom_entry_ref_count_offset);
        return NULL;
    }

    if (VMI_FAILURE == vmi_read_8_va(vmi, atom_addr + symbol_offsets.atom_entry_name_len_offset, 0, (uint8_t*)&entry->name_len))
    {
        printf("Error reading NameLength at %" PRIx64 "\n", atom_addr + symbol_offsets.atom_entry_name_len_offset);
        return NULL;
    }

    entry->name = read_wchar_str(vmi, atom_addr + symbol_offsets.atom_entry_name_offset, (size_t)entry->name_len);

    if (!entry->name)
    {
        printf("Error reading wchar-string Name at %" PRIx64 "\n", atom_addr + symbol_offsets.atom_entry_name_offset);
    }

    return entry;
}

struct atom_entry* create_atom_entry(uint16_t atom, const wchar_t* name, uint8_t len, addr_t hashlink, uint16_t refcount)
{
    struct atom_entry* entry = (struct atom_entry*) malloc(sizeof(struct atom_entry));
    memset(entry, 0, sizeof(struct atom_entry));
    entry->atom = atom;
...
GHashTable* build_atom_table(vmi_instance_t vmi, addr_t table_addr)
{
    ...
    if (VMI_FAILURE == vmi_read_32_va(vmi, table_addr + symbol_offsets.atom_table_num_buckets_off, 0, (uint32_t*)&num_buckets))
    {
        printf("Failed to read num buckets-value of _RTL_ATOM_TABLE at %" PRIx64 "\n", table_addr + symbol_offsets.atom_table_num_buckets_off);
        return NULL;
    }

    GHashTable* ht = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, (GDestroyNotify) free_atom_entry);
    ...
    while (i < num_buckets)
    {
        if (VMI_FAILURE == vmi_read_addr_va(vmi, table_addr + symbol_offsets.atom_table_buckets_off + i * 4, 0, &cur))
        {
