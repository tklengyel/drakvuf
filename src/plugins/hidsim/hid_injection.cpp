...
static int get_display_dimensions(qmp_connection* qc, dimensions* dims)
{
    ...
    matches = fscanf(fr, "%2s", ppm_hdr);

    /* Checks PPM-header */
    if (matches != 1 || strcmp(ppm_hdr, PPM_MAGIC_NUMBER) != 0)
    {
        fprintf(stderr, "[HIDSIM]: Error reading .ppm-files's magic number\n");
        fclose(fr);
        ret = -1;
    }
    ...
    matches = fscanf(fr, "%" SCNd32 "%" SCNd32, &width, &height);

    if (matches != 2 )
    {
        fprintf(stderr, "[HIDSIM]: Error reading .ppm-files metadata\n");
        fclose(fr);
        ret = -1;
    }

    /* Sanitizes read values; no need for a screen size larger than 65k */
    ...
    sig_atomic_t* has_to_stop)
{
    ...
    if (nr != 1)
    {
        fprintf(stderr, "[HIDSIM] Error reading first input event");
        return 1;
    }
