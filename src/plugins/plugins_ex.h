...
public:
    ...
    [[nodiscard]]
    std::unique_ptr<libhook::ReturnHook> createReturnHook(drakvuf_trap_info* info, Callback cb, const char* display_name = nullptr, uint64_t ttl = UNLIMITED_TTL);

    template<typename Params = PluginResult, typename Callback>
    [[nodiscard]]
    std::unique_ptr<libhook::SyscallHook> createSyscallHook(const std::string& syscall_name, Callback cb, const std::optional<std::string>& display_name = {}, uint64_t ttl = UNLIMITED_TTL);

    template<typename Params = PluginResult, typename Callback>
    [[nodiscard]]
    std::unique_ptr<libhook::Cr3Hook> createCr3Hook(Callback cb, uint64_t ttl = UNLIMITED_TTL);

    template<typename Params = PluginResult, typename Callback>
    [[nodiscard]]
    std::unique_ptr<libhook::CpuidHook> createCpuidHook(Callback cb, uint64_t ttl = UNLIMITED_TTL);

    template<typename Params = PluginResult, typename Callback>
    [[nodiscard]]
    std::unique_ptr<libhook::CatchAllHook> createCatchAllHook(Callback cb, uint64_t ttl = UNLIMITED_TTL);

    template<typename Params = PluginResult, typename Callback>
    [[nodiscard]]
    std::unique_ptr<libhook::MemAccessHook> createMemAccessHook(Callback cb, addr_t gfn, memaccess_type_t mem_access_type, vmi_mem_access_t mem_access, uint64_t ttl = UNLIMITED_TTL);
