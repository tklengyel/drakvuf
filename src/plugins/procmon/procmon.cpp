    ...
    reg_t status, vmi_pid_t new_pid, addr_t user_process_parameters_addr)
{
    ...
    {
        case OUTPUT_CSV:
            printf("procmon," FORMAT_TIMEVAL ",%" PRIu32 ",0x%" PRIx64 ",\"%s\",%" PRIi64",%s,0x%" PRIx64 ",%d,%s,%s,%s\n",
                   UNPACK_TIMEVAL(info->timestamp), info->vcpu, info->regs->cr3, info->proc_data.name,
                   info->proc_data.userid, info->trap->name, status, new_pid, cmdline, imagepath, dllpath);
            break;

        case OUTPUT_KV:
            printf("procmon Time=" FORMAT_TIMEVAL ",PID=%d,PPID=%d,ProcessName=\"%s\","
                   "Method=%s,Status=0x%" PRIx64 ",NewPid=%d,CommandLine=\"%s\",ImagePathName=\"%s\",DllPath=\"%s\"\n",
                   UNPACK_TIMEVAL(info->timestamp), info->proc_data.pid, info->proc_data.ppid, info->proc_data.name,
                   info->trap->name, status, new_pid, cmdline, imagepath, dllpath);
            break;

        default:
        case OUTPUT_DEFAULT:
            printf("[PROCMON] TIME:" FORMAT_TIMEVAL " VCPU:%" PRIu32 " CR3:0x%" PRIx64 ", EPROCESS:0x%" PRIx64
                   ", PID:%d, PPID:%d, \"%s\" %s:%" PRIi64 " %s:0x%" PRIx64 ":%d:%s:%s:%s\n",
                   ...
                   USERIDSTR(drakvuf), info->proc_data.userid, info->trap->name, status, new_pid, cmdline, imagepath, dllpath);
            break;
    }
