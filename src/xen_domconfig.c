...
int main(int argc, char **argv) {
    ...
    get_dom_info(xen, argv[1], &domID, &name);

    if (domID && name) {
        xen_domconfig_raw_t *config = xen_domconfig_raw_by_id(xen, domID);
        printf("%s", config->config_data);
        xen_free_domconfig_raw(config);
        free(name);
    }
